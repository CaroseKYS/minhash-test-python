...
class MinHash(object):
    ...
    def _init_permutations(self, num_perm):
        ...
        attr_name = f'permutations-of-seed-{self.seed}-{num_perm}'
        if not hasattr(MinHash, attr_name):
            gen = np.random.RandomState(self.seed)
            permutations = np.array([
                (gen.randint(1, _mersenne_prime, dtype=np.uint64), gen.randint(0, _mersenne_prime, dtype=np.uint64)) for
                _ in range(num_perm)
            ], dtype=np.uint64).T
            setattr(MinHash, attr_name, permutations)
        return getattr(MinHash, attr_name)
